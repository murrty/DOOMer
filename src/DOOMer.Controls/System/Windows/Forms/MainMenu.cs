...
///     Although <see cref="T:System.Windows.Forms.MenuStrip" /> replaces and adds functionality to the
///     <see cref="T:System.Windows.Forms.MainMenu" /> control of previous versions,
///     <see cref="T:System.Windows.Forms.MainMenu" /> is retained for both backward compatibility and future use if you choose.
/// </summary>
...
public class MainMenu : Menu {
    ...
    /// <summary>Initializes a new instance of the <see cref="T:System.Windows.Forms.MainMenu" /> with a specified set of <see cref="T:System.Windows.Forms.MenuItem" /> objects.</summary>
    /// <param name="frm">The form that the menu will be added on when the control is created.</param>
    ...
    /// <summary>
    /// Adds the menu to a form.
    /// </summary>
    /// <param name="frm">The form that the menu will be added to.</param>
    public void AddToForm(Form frm) {
        if (frm == form || frm.IsDisposed) {
            return;
        }
        ...
        form = frm;
        UnsafeNativeMethods.SetMenu(new HandleRef(frm, frm.Handle), new HandleRef(this, this.Handle));

        if (frm.MinimumSize.Height > 0) {
            frm.MinimumSize = new(frm.MinimumSize.Width, frm.MinimumSize.Height + 20);
        }
        if (frm.MaximumSize.Height > 0) {
            frm.MaximumSize = new(frm.MaximumSize.Width, frm.MaximumSize.Height + 20);
        }
        if ((frm.MinimumSize.Height == 0 && frm.MaximumSize.Height == 0)
        || frm.MaximumSize.Height > frm.MinimumSize.Height) {
            frm.Size = new(frm.Size.Width, frm.Size.Height + 20);
        }
    }
    /// <summary>
    /// Removes the menu from the form.
    /// </summary>
    public void RemoveFromForm() {
        if (form != null) {
            UnsafeNativeMethods.SetMenu(new HandleRef(form, form.Handle), NativeMethods.NullHandleRef);
            if (!form.IsDisposed) {
                if (form.MinimumSize.Height > 0) {
                    form.MinimumSize = new(form.MinimumSize.Width, form.MinimumSize.Height - 20);
                }
                if (form.MaximumSize.Height > 0) {
                    form.MaximumSize = new(form.MaximumSize.Width, form.MaximumSize.Height - 20);
                }
                if ((form.MinimumSize.Height == 0 && form.MaximumSize.Height == 0)
                || form.MaximumSize.Height > form.MinimumSize.Height) {
                    form.Size = new(form.Size.Width, form.Size.Height - 20);
                }
            }
        }
        form = null;
