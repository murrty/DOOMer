...
/// <summary>
///     Represents an individual item that is displayed within a <see cref="T:System.Windows.Forms.MainMenu" />
///     or <see cref="T:System.Windows.Forms.ContextMenu" />.
///     Although <see cref="T:System.Windows.Forms.ToolStripMenuItem" /> replaces and adds functionality to the
///     <see cref="T:System.Windows.Forms.MenuItem" /> control of previous versions,
///     <see cref="T:System.Windows.Forms.MenuItem" /> is retained for both backward compatibility and future use if you choose.
/// </summary>
[ToolboxItem(false)]
...
public class MenuItem : Menu {
    private static readonly Color AbsoluteTransparent = Color.FromArgb(0, 0, 0, 0);
    ...
    private class MdiListMoreWindowsData : MdiListUserData {
        ...
        public override void OnClick(EventArgs e) {
            Form[] array = parent.FindMdiForms();
            Form? activeMdiChild = parent.GetMainMenu()?.GetForm()?.ActiveMdiChild;
    ...
    private IntPtr hbmap;
    private Image? img;
    private MenuItemBitmapType bitmapType = MenuItemBitmapType.UserDefined;

    internal IntPtr Hbitmap => hbmap;

    /// <summary>
    /// Gets or sets the image visible on the menu item.
    /// <see cref="MenuItemBitmap"/> needs to be set to <see cref="MenuItemBitmapType.UserDefined"/>.
    /// </summary>
    [Browsable(true)]
    [DefaultValue(null)]
    public Image? Image {
        get => img;
        set {
            if (hbmap == 0 && value is null) {
                return;
            }

            if (hbmap != 0) {
                Natives.SafeNativeMethods.DeleteObject(new HandleRef(this, hbmap));
                hbmap = 0;
            }
            img = value;
            if (value != null) {
                using Bitmap renderer = new(value.Width, value.Height, Drawing.Imaging.PixelFormat.Format32bppPArgb);
                using Graphics drawer = Graphics.FromImage(renderer);
                drawer.DrawImage(value, 0, 0, value.Width, value.Height);
                hbmap = renderer.GetHbitmap(AbsoluteTransparent);
            }
            SetImage();
        }
    }
    ...
    /// <summary>
    /// Gets or sets the item bitmap visible on the menu item.
    /// </summary>
    [Browsable(true)]
    [DefaultValue(MenuItemBitmapType.UserDefined)]
    public MenuItemBitmapType MenuItemBitmap {
        get => bitmapType;
        set {
            if (bitmapType == value) {
                return;
    ...
    internal Menu? Menu {
        ...
        set {
            menu = value;
            if (hbmap != 0 || bitmapType != MenuItemBitmapType.UserDefined) {
                SetImage();
            }
        }
    }
    ...
    public MenuItem(MenuMerge mergeType, int mergeOrder, Shortcut shortcut, string? text, EventHandler? onClick, EventHandler? onPopup, EventHandler? onSelect, MenuItem[]? items) : base(items) {
        data = new MenuItemData(this, mergeType, mergeOrder, shortcut, showShortcut: true, text, onClick, onPopup, onSelect, null, null);
    }
    ...
    internal virtual void CreateMenuItem() {
        ...
        if ((data.State & 0x10000) == 0) {
            ...
            // The hbmp does not get respected, so it has to be re-set.
            SetImageOnMenuItemInfo(ref mENUITEMINFO_T, menuHandle);
    ...
    internal static MenuItem? GetMenuItemFromItemData(IntPtr itemData) {
        ...
        if (IntPtr.Size == 4) {
            if (num < 3221225472u) {
                num = (Marshal.PtrToStructure<MsaaMenuInfoWithId>(itemData)).uniqueID;
            }
        }
        else {
            num = (Marshal.PtrToStructure<MsaaMenuInfoWithId>(itemData)).uniqueID;
    ...
    private void PopulateMdiList() {
        ...
        try {
            ...
            if (MenuItems.Count > 0) {
                MenuItem menuItem = Activator.CreateInstance<MenuItem>();
            ...
            for (int i = 0; i < array.Length; i++) {
                ...
                if ((flag && num3 < 9) || (!flag && num3 < 8) || array[i].Equals(activeMdiChild)) {
                    MenuItem menuItem2 = Activator.CreateInstance<MenuItem>();
            ...
            if (num > 9) {
                MenuItem menuItem3 = Activator.CreateInstance<MenuItem>();
    ...
    internal void UpdateMenuItem(bool force) {
        ...
        dataVersion = data.version;
        if (menu is MainMenu) {
            Form? formUnsafe = ((MainMenu)menu).GetForm();
            if (formUnsafe != null) {
    ...
    private void SetImageOnMenuItemInfo(ref NativeMethods.MENUITEMINFO_T mENUITEMINFO_T, HandleRef menuHandle) {
        if (menu is not null && (hbmap != IntPtr.Zero || bitmapType != MenuItemBitmapType.UserDefined)) {
            int VisibleIndex = 0;
            for (int i = 0; i < menu.MenuItems.Count; i++) {
                ...
                if (m == this) {
                    ...
                    mENUITEMINFO_T.fMask = 0x80;
                    mENUITEMINFO_T.hbmpChecked = 0;
                    mENUITEMINFO_T.hbmpUnchecked = 0;
                    mENUITEMINFO_T.hbmpItem = GetBmpPtr();
                    UnsafeNativeMethods.SetMenuItemInfo(menuHandle, VisibleIndex, true, mENUITEMINFO_T);
                    break;
                }
                if (m.Visible) {
                    VisibleIndex++;
    ...
    private void SetImage() {
        if (menu == null || !menu.created || !Visible) {
            return;
        }

        HandleRef menuHandle = new(menu, menu.handle);
        var menuInfo = new NativeMethods.MENUITEMINFO_T_RW();

        int VisibleIndex = 0;
        for (int i = 0; i < menu.MenuItems.Count; i++) {
            MenuItem m = menu.MenuItems[i];
            if (m == this) {
                _ = UnsafeNativeMethods.GetMenuItemInfo(menuHandle, VisibleIndex, true, menuInfo);
                menuInfo.fMask = 0x80;
                menuInfo.hbmpChecked = 0;
                menuInfo.hbmpUnchecked = 0;
                menuInfo.hbmpItem = GetBmpPtr();
                _ = UnsafeNativeMethods.SetMenuItemInfo(menuHandle, VisibleIndex, true, menuInfo);
                break;
            }
            if (m.Visible) {
                VisibleIndex++;
        ...
        //_ = UnsafeNativeMethods.GetMenuItemInfo(menuHandle, this.Index, true, menuInfo);
        //menuInfo.fMask = 0x80;
        //menuInfo.hbmpChecked = 0;
        //menuInfo.hbmpUnchecked = 0;
        //menuInfo.hbmpItem = GetBmpPtr();
        //_ = UnsafeNativeMethods.SetMenuItemInfo(menuHandle, this.Index, true, menuInfo);
    }

    internal IntPtr GetBmpPtr() {
        return bitmapType switch {
            MenuItemBitmapType.MenuCallback => -1,
            MenuItemBitmapType.System => 1,
            MenuItemBitmapType.MbarRestore => 2,
            MenuItemBitmapType.MbarMinimize => 3,
            MenuItemBitmapType.MbarClose => 5,
            MenuItemBitmapType.MbarCloseDisabled => 6,
            MenuItemBitmapType.MbarMinimizeDisabled => 7,
            MenuItemBitmapType.PopupClose => 8,
            MenuItemBitmapType.PopupRestore => 9,
            MenuItemBitmapType.PopupMaximize => 10,
            MenuItemBitmapType.PopupMinimized => 11,
            _ => hbmap
        };
    }
}
