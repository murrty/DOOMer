...
public class SevenZipExtractor : IExtractor {
    ...
    public IReadOnlyList<ArchiveFile> Files { get; private set; }

    public FeedbackNeededDelegate OverwriteDelegate { get; set; }

    public ProgressDelegate ProgressDelegate { get; set; }

    public bool AllowFileDeletions { get; set; }
    ...
    public IExtractor ExtractFile(UInt64 index, string outputDirectory) {
        ...
        if (!preProcessFile(outputDirectory, file)) {
            string fullPath = Path.Combine(outputDirectory, PreserveDirectoryStructure ? file.Name : file.Name);
